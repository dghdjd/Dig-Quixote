...
void RenderSystem::initializeGlMeshes()
{
	for (uint i = 0; i < mesh_paths.size(); i++)
	{
		...
		//std::cout << "renderSysinit.cpp Loaded mesh for ID: " << (int)geom_index << ". Vertex count: " << meshes[(int)geom_index].vertices.size() << std::endl;
	}
}

void RenderSystem::setupPlayerAnimation(float width, GEOMETRY_BUFFER_ID bufferID)
{
	std::vector<TexturedVertex> textured_vertices1(4);
	width = SpriteSheetWidths::PLAYER_IDLE;


	textured_vertices1[0].position = { -1.f / 2, +1.f / 2, 0.f };
	textured_vertices1[1].position = { +1.f / 2, +1.f / 2, 0.f };
	textured_vertices1[2].position = { +1.f / 2, -1.f / 2, 0.f };
	textured_vertices1[3].position = { -1.f / 2, -1.f / 2, 0.f };

	textured_vertices1[0].texcoord = { (50.f / width), 1.f };
	textured_vertices1[1].texcoord = { (100.f / width), 1.f };		//550
	textured_vertices1[2].texcoord = { (100.f / width), 0.f };
	textured_vertices1[3].texcoord = { (50.f / width), 0.f };

	// Counterclockwise as it's the default opengl front winding direction.
	const std::vector<uint16_t> textured_indices1 = { 0, 3, 1, 1, 3, 2 };
	bindVBOandIBO(bufferID, textured_vertices1, textured_indices1);
}

void RenderSystem::setupNumbers()
{
	std::vector<TexturedVertex> textured_vertices1(4);
	...
	textured_vertices1[0].texcoord = { 0.f, 1.f };  //0
	textured_vertices1[1].texcoord = { 1.f, 1.f };  //1
	textured_vertices1[2].texcoord = { 1.f, 0.f };  //1
	textured_vertices1[3].texcoord = { 0.f, 0.f };  //0

	// Counterclockwise as it's the default opengl front winding direction.
	const std::vector<uint16_t> textured_indices1 = { 0, 3, 1, 1, 3, 2 };
	bindVBOandIBO(GEOMETRY_BUFFER_ID::NUMBERS, textured_vertices1, textured_indices1);
}
...
void RenderSystem::initializeGlGeometryBuffers()
{
	...
	setupPlayerAnimation(SpriteSheetWidths::PLAYER_IDLE,   GEOMETRY_BUFFER_ID::PLAYER_IDLE);
	setupPlayerAnimation(SpriteSheetWidths::PLAYER_RUN,    GEOMETRY_BUFFER_ID::PLAYER_RUN);
	setupPlayerAnimation(SpriteSheetWidths::PLAYER_JUMP,   GEOMETRY_BUFFER_ID::PLAYER_JUMP);
	setupPlayerAnimation(SpriteSheetWidths::PLAYER_ATTACK, GEOMETRY_BUFFER_ID::PLAYER_ATTACK);


	std::vector<TexturedVertex> textured_vertices2(4);
	textured_vertices2[0].position = { -1.f / 2, +1.f / 2, 0.f };
	textured_vertices2[1].position = { +1.f / 2, +1.f / 2, 0.f };
	textured_vertices2[2].position = { +1.f / 2, -1.f / 2, 0.f };
	textured_vertices2[3].position = { -1.f / 2, -1.f / 2, 0.f };
	textured_vertices2[0].texcoord = { 0.f, 1.f };  //0
	textured_vertices2[1].texcoord = { 64.f / 512.f, 1.f };  //1
	textured_vertices2[2].texcoord = { 64.f / 512.f, 0.f };  //1
	textured_vertices2[3].texcoord = { 0.f, 0.f };  //0
	// Counterclockwise as it's the default opengl front winding direction.
	const std::vector<uint16_t> textured_indices2 = { 0, 3, 1, 1, 3, 2 };
	bindVBOandIBO(GEOMETRY_BUFFER_ID::WATER, textured_vertices2, textured_indices2);

	std::vector<TexturedVertex> textured_vertices24(4);
	textured_vertices24[0].position = { -1.f / 2, +1.f / 2, 0.f };
	textured_vertices24[1].position = { +1.f / 2, +1.f / 2, 0.f };
	textured_vertices24[2].position = { +1.f / 2, -1.f / 2, 0.f };
	textured_vertices24[3].position = { -1.f / 2, -1.f / 2, 0.f };
	textured_vertices24[0].texcoord = { 0.f, 1.f };  //0
	textured_vertices24[1].texcoord = { 498.f / 9960.f, 1.f };  //1
	textured_vertices24[2].texcoord = { 498.f / 9960.f, 0.f };  //1
	textured_vertices24[3].texcoord = { 0.f, 0.f };  //0
	// Counterclockwise as it's the default opengl front winding direction.
	const std::vector<uint16_t> textured_indices24 = { 0, 3, 1, 1, 3, 2 };
	bindVBOandIBO(GEOMETRY_BUFFER_ID::PORTAL, textured_vertices24, textured_indices24);

	std::vector<TexturedVertex> textured_vertices244(4);
	textured_vertices244[0].position = { -1.f / 2, +1.f / 2, 0.f };
	textured_vertices244[1].position = { +1.f / 2, +1.f / 2, 0.f };
	textured_vertices244[2].position = { +1.f / 2, -1.f / 2, 0.f };
	textured_vertices244[3].position = { -1.f / 2, -1.f / 2, 0.f };
	textured_vertices244[0].texcoord = { 0.f, 1.f };  //0
	textured_vertices244[1].texcoord = { 256.f / 2048.f, 1.f };  //1
	textured_vertices244[2].texcoord = { 256.f / 9960.f, 0.f };  //1
	textured_vertices244[3].texcoord = { 0.f, 0.f };  //0
	// Counterclockwise as it's the default opengl front winding direction.
	const std::vector<uint16_t> textured_indices244 = { 0, 3, 1, 1, 3, 2 };
	bindVBOandIBO(GEOMETRY_BUFFER_ID::LIGHTING, textured_vertices24, textured_indices24);
	...
	for (int i = 0; i < NUM_TRIANGLES; i++) {
		...
		pebble_vertices.back().color = { 0.9, 0.9, 0.9 };
	...
	int geom_index = (int)GEOMETRY_BUFFER_ID::PARTICLE;
	meshes[geom_index].vertices = pebble_vertices;
	meshes[geom_index].vertex_indices = pebble_indices;
	bindVBOandIBO(GEOMETRY_BUFFER_ID::PARTICLE, meshes[geom_index].vertices, meshes[geom_index].vertex_indices);
