...
#pragma warning(disable : 4996)
	...
	std::cout << "Loaded " << out_vertices.size() << " vertices from " << obj_path << std::endl;

	// std::cout << "Inside extractPolygonVertices, vertices: " << std::endl;
	// for (const ColoredVertex &v : out_vertices)
	// {
...
std::vector<vec2> Mesh::extractPolygonVertices() const
{
	...
	{
		result.push_back(vec2(vertex.position.x, vertex.position.y));
	}

	// std::cout << "Inside extractPolygonVertices, vertices: " << std::endl;
	// for(const vec2& v : result)
	// {
